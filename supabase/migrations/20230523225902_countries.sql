create table "public"."countries" (
    "id" INT not null,
    "code" VARCHAR(3),
    "name" VARCHAR(512)
);

alter table "public"."countries" enable row level security;

CREATE UNIQUE INDEX countries_id_key ON public.countries USING btree (id);
CREATE UNIQUE INDEX countries_pkey ON public.countries USING btree (id);

alter table "public"."countries" add constraint "countries_id_key" UNIQUE using index "countries_id_key";
alter table "public"."countries" add constraint "countries_pkey" PRIMARY KEY using index "countries_pkey";

alter table "public"."user_data" add column "country" INT;
alter table "public"."user_data" add constraint "user_data_country_fkey" FOREIGN KEY (country) REFERENCES countries(id) not valid;
alter table "public"."user_data" validate constraint "user_data_country_fkey";

INSERT INTO "countries" ("id", "code", "name") VALUES
	('1', 'AF', 'Afghanistan'),
	('3', 'AL', 'Albania'),
	('4', 'DZ', 'Algeria'),
	('6', 'AD', 'Andorra'),
	('7', 'AO', 'Angola'),
	('10', 'AG', 'Antigua And Barbuda'),
	('11', 'AR', 'Argentina'),
	('12', 'AM', 'Armenia'),
	('14', 'AU', 'Australia'),
	('15', 'AT', 'Austria'),
	('16', 'AZ', 'Azerbaijan'),
	('18', 'BH', 'Bahrain'),
	('19', 'BD', 'Bangladesh'),
	('20', 'BB', 'Barbados'),
	('21', 'BY', 'Belarus'),
	('22', 'BE', 'Belgium'),
	('23', 'BZ', 'Belize'),
	('24', 'BJ', 'Benin'),
	('25', 'BM', 'Bermuda'),
	('26', 'BT', 'Bhutan'),
	('27', 'BO', 'Bolivia'),
	('155', 'BQ', 'Bonaire, Sint Eustatius and Saba'),
	('28', 'BA', 'Bosnia and Herzegovina'),
	('29', 'BW', 'Botswana'),
	('31', 'BR', 'Brazil'),
	('33', 'BN', 'Brunei'),
	('34', 'BG', 'Bulgaria'),
	('35', 'BF', 'Burkina Faso'),
	('36', 'BI', 'Burundi'),
	('37', 'KH', 'Cambodia'),
	('38', 'CM', 'Cameroon'),
	('39', 'CA', 'Canada'),
	('40', 'CV', 'Cape Verde'),
	('42', 'CF', 'Central African Republic'),
	('43', 'TD', 'Chad'),
	('44', 'CL', 'Chile'),
	('45', 'CN', 'China'),
	('48', 'CO', 'Colombia'),
	('49', 'KM', 'Comoros'),
	('50', 'CG', 'Congo'),
	('53', 'CR', 'Costa Rica'),
	('54', 'CI', 'Cote D''Ivoire (Ivory Coast)'),
	('55', 'HR', 'Croatia'),
	('56', 'CU', 'Cuba'),
	('57', 'CY', 'Cyprus'),
	('58', 'CZ', 'Czech Republic'),
	('51', 'CD', 'Democratic Republic of the Congo'),
	('59', 'DK', 'Denmark'),
	('60', 'DJ', 'Djibouti'),
	('61', 'DM', 'Dominica'),
	('62', 'DO', 'Dominican Republic'),
	('63', 'TL', 'East Timor'),
	('64', 'EC', 'Ecuador'),
	('65', 'EG', 'Egypt'),
	('66', 'SV', 'El Salvador'),
	('67', 'GQ', 'Equatorial Guinea'),
	('68', 'ER', 'Eritrea'),
	('69', 'EE', 'Estonia'),
	('70', 'ET', 'Ethiopia'),
	('73', 'FJ', 'Fiji Islands'),
	('74', 'FI', 'Finland'),
	('75', 'FR', 'France'),
	('79', 'GA', 'Gabon'),
	('80', 'GM', 'Gambia The'),
	('81', 'GE', 'Georgia'),
	('82', 'DE', 'Germany'),
	('83', 'GH', 'Ghana'),
	('85', 'GR', 'Greece'),
	('87', 'GD', 'Grenada'),
	('90', 'GT', 'Guatemala'),
	('92', 'GN', 'Guinea'),
	('93', 'GW', 'Guinea-Bissau'),
	('94', 'GY', 'Guyana'),
	('95', 'HT', 'Haiti'),
	('97', 'HN', 'Honduras'),
	('98', 'HK', 'Hong Kong S.A.R.'),
	('99', 'HU', 'Hungary'),
	('100', 'IS', 'Iceland'),
	('101', 'IN', 'India'),
	('102', 'ID', 'Indonesia'),
	('103', 'IR', 'Iran'),
	('104', 'IQ', 'Iraq'),
	('105', 'IE', 'Ireland'),
	('106', 'IL', 'Israel'),
	('107', 'IT', 'Italy'),
	('108', 'JM', 'Jamaica'),
	('109', 'JP', 'Japan'),
	('111', 'JO', 'Jordan'),
	('112', 'KZ', 'Kazakhstan'),
	('113', 'KE', 'Kenya'),
	('114', 'KI', 'Kiribati'),
	('248', 'XK', 'Kosovo'),
	('117', 'KW', 'Kuwait'),
	('118', 'KG', 'Kyrgyzstan'),
	('119', 'LA', 'Laos'),
	('120', 'LV', 'Latvia'),
	('121', 'LB', 'Lebanon'),
	('122', 'LS', 'Lesotho'),
	('123', 'LR', 'Liberia'),
	('124', 'LY', 'Libya'),
	('125', 'LI', 'Liechtenstein'),
	('126', 'LT', 'Lithuania'),
	('127', 'LU', 'Luxembourg'),
	('130', 'MG', 'Madagascar'),
	('131', 'MW', 'Malawi'),
	('132', 'MY', 'Malaysia'),
	('133', 'MV', 'Maldives'),
	('134', 'ML', 'Mali'),
	('135', 'MT', 'Malta'),
	('137', 'MH', 'Marshall Islands'),
	('139', 'MR', 'Mauritania'),
	('140', 'MU', 'Mauritius'),
	('142', 'MX', 'Mexico'),
	('143', 'FM', 'Micronesia'),
	('144', 'MD', 'Moldova'),
	('145', 'MC', 'Monaco'),
	('146', 'MN', 'Mongolia'),
	('147', 'ME', 'Montenegro'),
	('149', 'MA', 'Morocco'),
	('150', 'MZ', 'Mozambique'),
	('151', 'MM', 'Myanmar'),
	('152', 'NA', 'Namibia'),
	('153', 'NR', 'Nauru'),
	('154', 'NP', 'Nepal'),
	('156', 'NL', 'Netherlands'),
	('158', 'NZ', 'New Zealand'),
	('159', 'NI', 'Nicaragua'),
	('160', 'NE', 'Niger'),
	('161', 'NG', 'Nigeria'),
	('115', 'KP', 'North Korea'),
	('129', 'MK', 'North Macedonia'),
	('165', 'NO', 'Norway'),
	('166', 'OM', 'Oman'),
	('167', 'PK', 'Pakistan'),
	('168', 'PW', 'Palau'),
	('169', 'PS', 'Palestinian Territory Occupied'),
	('170', 'PA', 'Panama'),
	('171', 'PG', 'Papua new Guinea'),
	('172', 'PY', 'Paraguay'),
	('173', 'PE', 'Peru'),
	('174', 'PH', 'Philippines'),
	('176', 'PL', 'Poland'),
	('177', 'PT', 'Portugal'),
	('178', 'PR', 'Puerto Rico'),
	('179', 'QA', 'Qatar'),
	('181', 'RO', 'Romania'),
	('182', 'RU', 'Russia'),
	('183', 'RW', 'Rwanda'),
	('185', 'KN', 'Saint Kitts And Nevis'),
	('186', 'LC', 'Saint Lucia'),
	('188', 'VC', 'Saint Vincent And The Grenadines'),
	('191', 'WS', 'Samoa'),
	('192', 'SM', 'San Marino'),
	('193', 'ST', 'Sao Tome and Principe'),
	('194', 'SA', 'Saudi Arabia'),
	('195', 'SN', 'Senegal'),
	('196', 'RS', 'Serbia'),
	('197', 'SC', 'Seychelles'),
	('198', 'SL', 'Sierra Leone'),
	('199', 'SG', 'Singapore'),
	('200', 'SK', 'Slovakia'),
	('201', 'SI', 'Slovenia'),
	('202', 'SB', 'Solomon Islands'),
	('203', 'SO', 'Somalia'),
	('204', 'ZA', 'South Africa'),
	('116', 'KR', 'South Korea'),
	('206', 'SS', 'South Sudan'),
	('207', 'ES', 'Spain'),
	('208', 'LK', 'Sri Lanka'),
	('209', 'SD', 'Sudan'),
	('210', 'SR', 'Suriname'),
	('212', 'SZ', 'Swaziland'),
	('213', 'SE', 'Sweden'),
	('214', 'CH', 'Switzerland'),
	('215', 'SY', 'Syria'),
	('216', 'TW', 'Taiwan'),
	('217', 'TJ', 'Tajikistan'),
	('218', 'TZ', 'Tanzania'),
	('219', 'TH', 'Thailand'),
	('17', 'BS', 'The Bahamas'),
	('220', 'TG', 'Togo'),
	('222', 'TO', 'Tonga'),
	('223', 'TT', 'Trinidad And Tobago'),
	('224', 'TN', 'Tunisia'),
	('225', 'TR', 'Turkey'),
	('226', 'TM', 'Turkmenistan'),
	('228', 'TV', 'Tuvalu'),
	('229', 'UG', 'Uganda'),
	('230', 'UA', 'Ukraine'),
	('231', 'AE', 'United Arab Emirates'),
	('232', 'GB', 'United Kingdom'),
	('233', 'US', 'United States'),
	('234', 'UM', 'United States Minor Outlying Islands'),
	('235', 'UY', 'Uruguay'),
	('236', 'UZ', 'Uzbekistan'),
	('237', 'VU', 'Vanuatu'),
	('239', 'VE', 'Venezuela'),
	('240', 'VN', 'Vietnam'),
	('242', 'VI', 'Virgin Islands (US)'),
	('245', 'YE', 'Yemen'),
	('246', 'ZM', 'Zambia'),
	('247', 'ZW', 'Zimbabwe');