...
class MyndSupabaseUser extends BaseAuthUser {
  MyndSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> myndSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = MyndSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
